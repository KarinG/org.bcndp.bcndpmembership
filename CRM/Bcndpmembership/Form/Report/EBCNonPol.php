 ...
 *
 */
class CRM_Bcndpmembership_Form_Report_EBCNonPol extends CRM_Report_Form {
...
//            'required' => TRUE,
      ...
      'temp_civicrm_contribution_aggregates' =>
      array(
        'dao' => 'CRM_Contribute_DAO_Contribution',
        'alias' => 'aggregate_amount',
        'fields' =>
        array(
          'total' =>
          array(
            'title' => ts('Donor Total'),
  ...
  function from($build_subquery = FALSE) {
    ...
    if (!$build_subquery) {
      $this->_from .= "
              INNER JOIN temp_civicrm_contribution_aggregates {$this->_aliases['temp_civicrm_contribution_aggregates']}
                      ON {$this->_aliases['temp_civicrm_contribution_aggregates']}.id = {$this->_aliases['civicrm_contact']}.id";
    }
  ...
  function postProcess() {
    ...
    // JMW first generate a temporary table that includes aggregate totals per donor. this will be joined
    // into the main query below. apply same filters to main query and subquery.
    if (isset($this->_columns['temp_civicrm_contribution_aggregates']['filters']['total'])) {
      $this->_columns['temp_civicrm_contribution_aggregates']['filters']['total']['pseudofield'] = 1;
    }
    $this->from(TRUE);
    $this->customDataFrom();
    $this->where();
    $agg_from = $this->_from;
    $agg_where = $this->_where;
    if (isset($this->_columns['temp_civicrm_contribution_aggregates']['filters']['total'])) {
      $this->_columns['temp_civicrm_contribution_aggregates']['filters']['total']['pseudofield'] = 0;
    }
    $this->_from = ''; $this->_where = ''; $this->_whereClauses = array();

    $sql = "SELECT contact_civireport.id as id, SUM(contribution_civireport.total_amount) as total {$agg_from} {$agg_where} GROUP BY contact_civireport.id";
    $tempQuery = 'CREATE TEMPORARY TABLE temp_civicrm_contribution_aggregates AS ' . $sql;
    CRM_Core_DAO::executeQuery($tempQuery);
  
    // JMW then finish building the report as normal
    $sql = $this->buildQuery();
  ...
  function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      // KG strip the : if it exists and the page number behind it
      if (array_key_exists('civicrm_value_additional_contribution_info_29_custom_664', $row)) {
        if ($value = $row['civicrm_value_additional_contribution_info_29_custom_664']) {
          $nopagenum = explode(":", $value);
          $rows[$rowNum]['civicrm_value_additional_contribution_info_29_custom_664'] = $nopagenum[0];
        }
      }

      // KG strip the Mr. Ms. or Mrs. or Dr. Miss Rev. Capt. Prof. Hon. Rt. Hon. from display_name
      if (array_key_exists('civicrm_contact_display_name', $row)) {
        if ($value = $row['civicrm_contact_display_name']) {
          $prefixes = array("Mr.", "Ms.", "Mrs.", "Dr.", "Miss", "Rev.", "Capt.", "Prof.", "Hon.", "Rt. Hon.");
          $noprefix = str_replace($prefixes, "", $value);
//          $noprefix = ltrim($value, $prefixes);
          $postfixes = array("MLA", "MP");
          $nopostfix = str_replace($postfixes, '', $noprefix);
          $rows[$rowNum]['civicrm_contact_display_name'] = trim($nopostfix);
        }
      }

      // KG rewrite contact subtype to class
      // Individual: class 1
      // Default: class 2
      // Unincorporated_Business: class 3 //
      // Union: class 4 //
      // Non_Profit: class 5
      // ClassSix: class 6

      if (array_key_exists('civicrm_contact_contact_type', $row) && $value = $row['civicrm_contact_contact_type']) {
        if ($value == 'Individual') {
          $class = 1;
          $rows[$rowNum]['civicrm_contact_contact_sub_type'] = $class;
        } else {
          $class = 2;

          if (array_key_exists('civicrm_contact_contact_sub_type', $row) && $value_sub = $row['civicrm_contact_contact_sub_type']) {

            if (strpos($value_sub, 'ClassSix') !== false) {
              $class = 6;
            };
            if (strpos($value_sub, 'Union') !== false) {
              $class = 4;
            };
            if (strpos($value_sub, 'Non_Profit') !== false) {
              $class = 5;
            };
            if (strpos($value_sub, 'Unincorporated_Business') !== false) {
              $class = 3;
            };
            $rows[$rowNum]['civicrm_contact_contact_sub_type'] = $class;
          }
