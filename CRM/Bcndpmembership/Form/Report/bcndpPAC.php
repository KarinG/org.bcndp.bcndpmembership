 ...
 *
 */
class CRM_Bcndpmembership_Form_Report_bcndpPAC extends CRM_Report_Form {
  ...
  protected $_customGroupExtends = array(
    'Contribution', 'Individual', 'Contact'); function __construct() {
...
$this->_groupBy = " GROUP BY {$this->_aliases['civicrm_contact']}.id ";
  ...
  function alterDisplay(&$rows) {
    ...
    // print_r($row);
      ...
      // CRM_Utils_Crypt::decrypt($dao->credit_card_nbr_258)
      if (array_key_exists('civicrm_value_recurring_32_custom_258', $row)) {
        if ($value = $row['civicrm_value_recurring_32_custom_258']) {
          $rows[$rowNum]['civicrm_value_recurring_32_custom_258'] = CRM_Utils_Crypt::decrypt($value);
          $entryFound = TRUE;
        }
      }

      // decrypt bank account number
      // civicrm_value_recurring_32_custom_309
      if (array_key_exists('civicrm_value_recurring_32_custom_309', $row)) {
        if ($value = $row['civicrm_value_recurring_32_custom_309']) {
          $rows[$rowNum]['civicrm_value_recurring_32_custom_309'] = CRM_Utils_Crypt::decrypt($value);
          $entryFound = TRUE;
        }
      }
