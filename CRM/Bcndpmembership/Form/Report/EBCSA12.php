 ...
 *
 */
class CRM_Bcndpmembership_Form_Report_EBCSA12 extends CRM_Report_Form {
...
//            'required' => TRUE,
      ...
      array(
        ...
        array(
          ...
          'total_amount_aggregate' =>
          array('title' => ts('Contribution Aggregate'),
            'default' => TRUE,
            'dbAlias' => "SUM(contribution_civireport.total_amount)",
//            'dbAlias' => "SUM(CASE WHEN year(contribution_civireport.receive_date)=2016 THEN contribution_civireport.total_amount ELSE 0 END)",
          ),
        ),
        'filters' =>
        array(
...
//                     ON {$this->_aliases['civicrm_value_additional_contribution_info_29']}.entity_id = {$this->_aliases['civicrm_contribution']}.id\n";
...
GROUP BY {$this->_aliases['civicrm_contribution']}.currency";
  ...
  function postProcess() {
    ...
    // JMW build a table of contribution totals per contact so we can include the Contribution Aggregate
    $sql = "SELECT contact_civireport.id as contact_id, SUM(contribution_civireport.total_amount) as aggregate_amount {$this->_from} {$this->_where} GROUP BY contact_civireport.id";
    $tempQuery = 'CREATE TEMPORARY TABLE civireport_contribution_detail_temptotals AS ' . $sql;
    CRM_Core_DAO::executeQuery($tempQuery);
...
(SELECT * FROM civireport_contribution_detail_temp2)";
  ...
  function alterDisplay(&$rows) {
    ...
    // JMW query and store the contribution totals
    $sql = "SELECT contact_id, aggregate_amount FROM civireport_contribution_detail_temptotals";
    $dao = CRM_Core_DAO::executeQuery($sql);
    while ($dao->fetch()) {
      $aggregate_amount[$dao->contact_id] = $dao->aggregate_amount;
    }
...
//          $noprefix = ltrim($value, $prefixes);
      ...
      if (array_key_exists('civicrm_contact_contact_type', $row)) {
        if ($value = $row['civicrm_contact_contact_type']) {
          if ($value == 'Individual') {$class = 1;}
          if ($value == 'Organization') {$class = 2;}

          if (array_key_exists('civicrm_contact_contact_sub_type', $row)) {

            if ($value_sub = $row['civicrm_contact_contact_sub_type']) {
              if ($value_sub == 'Unincorporated Business')  {$class = 3;}
              if ($value_sub == 'Union')  {$class = 4;}
              if ($value_sub == 'Non_Profit')  {$class = 5;}
              if ($value_sub == 'ClassSix')  {$class = 6;}
      ...
      // JMW provide aggregate amount
      if ($value = CRM_Utils_Array::value('civicrm_contribution_total_amount_aggregate', $row)) {
        $cid = $row['civicrm_contact_id'];
        $rows[$rowNum]['civicrm_contribution_total_amount_aggregate'] = CRM_Utils_Money::format($aggregate_amount[$cid]);
        $entryFound = TRUE;
      }
